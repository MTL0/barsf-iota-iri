...
import com.iota.iri.storage.innoDB.mybatis.modelMapper.StateDiffMapper;
...
public class StateDiffProvider implements SubPersistenceProvider {
    ...
    public boolean save(Persistable thing, Indexable index, SqlSession outSession, boolean mergeMode, AtomicLong al) {
        ...
        StateDiff target = (StateDiff) thing;
    ...
    public boolean save(byte[] thing, Indexable index, SqlSession outSession, boolean mergeMode) throws Exception {
        ...
        StateDiffMapper mapper = outSession.getMapper(StateDiffMapper.class);
        StateDiff target = StateDiff.class.newInstance();
//        target.read(thing);
    ...
    public boolean save(Persistable thing, Indexable index) {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            ...
            ((StateDiff) thing).filling(index);
            mapper.insertOrUpdate((StateDiff) thing);
        }
        return true;
    }
    ...
    public boolean update(Persistable model, Indexable index, String item) throws Exception {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            StateDiffMapper mapper = session.getMapper(StateDiffMapper.class);
            ((StateDiff) model).filling(index);
            int effect = mapper.updateByPrimaryKey((StateDiff) model);
            return true;
        }
    }
    ...
    public boolean exists(Indexable key) throws Exception {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            StateDiffMapper mapper = session.getMapper(StateDiffMapper.class);
            StateDiff object = mapper.selectByPrimaryKey(converterIndexableToStr(key));
            return object != null;
        }
    }
    ...
    public Persistable get(Indexable index) throws Exception {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            StateDiffMapper mapper = session.getMapper(StateDiffMapper.class);
            StateDiff object = mapper.selectByPrimaryKey(converterIndexableToStr(index));
            return object != null ? object : StateDiff.class.newInstance();
        }
    }
    ...
    public Pair<Indexable, Persistable> next(Indexable index) throws Exception {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            StateDiffMapper mapper = session.getMapper(StateDiffMapper.class);
            StateDiff value = mapper.next(converterIndexableToStr(index));
            if (value != null) {
                // todo 这里没有使用类型创建对象
                Indexable indexNext;
                if (StringUtils.isNumeric(value.getHash())) {
    ...
    public Pair<Indexable, Persistable> previous(Indexable index) throws Exception {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            StateDiffMapper mapper = session.getMapper(StateDiffMapper.class);
            StateDiff value = mapper.previous(converterIndexableToStr(index));
            if (value != null) {
                // todo 这里没有使用类型创建对象
                Indexable indexNext;
                if (StringUtils.isNumeric(value.getHash())) {
    ...
    public void saveUnUpdate(Persistable thing, Indexable index) {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            StateDiffMapper mapper = session.getMapper(StateDiffMapper.class);
            ((StateDiff)thing).filling(index);
            mapper.insert((StateDiff) thing);
        }
    }
}
