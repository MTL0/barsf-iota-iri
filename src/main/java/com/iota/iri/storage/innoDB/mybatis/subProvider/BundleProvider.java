...
import com.iota.iri.storage.innoDB.mybatis.modelMapper.BundleMapper;
...
//        return new Pair<>(null, null);
    ...
    public Persistable get(Indexable index) throws Exception {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            ...
            List<Transaction> hashOfTransByBundle = mapper.selectByExample(selectHash);
            if (hashOfTransByBundle.size() > 0) {
                return new Bundle(hashOfTransByBundle.stream().map(t -> new Hash(t.getHash())).collect(Collectors.toList()));
            } else {
