...
import com.iota.iri.storage.innoDB.mybatis.modelMapper.ApproveeMapper;
...
public class ApproveeProvider implements SubPersistenceProvider {
    ...
    public Persistable get(Indexable index) throws Exception {
        try (final SqlSession session = this.sessionFactory.openSession(true)) {
            ...
            if (hashOfTransByTag.size() > 0) {
                return new Approvee(hashOfTransByTag.stream().map(t -> new Hash(t.getHash())).collect(Collectors.toList()));
            } else {
    ...
    public void saveUnUpdate(Persistable thing, Indexable index, SqlSession session) {
        ApproveeMapper mapper = session.getMapper(ApproveeMapper.class);
        mapper.insert((Approvee) thing);
    }
}
