...
public class RodbDataFishingRunner implements Runnable {
    private static final Logger log = LoggerFactory.getLogger(RodbDataFishingRunner.class);
...
//                                    break;
    ...
    private KvEnum tellMeWhatIsTheKey(Class nowClass) {
        KvEnum key;
        if (nowClass == Transaction.class) {
            key = KvEnum.transaction;
        } else if (nowClass == Milestone.class) {
            key = KvEnum.milestone;
        } else if (nowClass == StateDiff.class) {
            key = KvEnum.statediff;
    ...
    private void initLastOneMap() {
        ...
        Kv transactionIndex = kvProvider.getValue(KvEnum.transaction.name());
        lastIndexMAp.put(Transaction.class, transactionIndex == null ? Hash.NULL_HASH : new Hash(transactionIndex.getValueStr()));
        Kv milestoneIndex = kvProvider.getValue(KvEnum.milestone.name());
