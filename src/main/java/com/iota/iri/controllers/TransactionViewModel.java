...
//        hashesList.add(new Pair<>(getTagValue(), new Tag(hash)));
    ...
    //对源码进行了修改, 新添的方法
    public int store(Tangle tangle, boolean innodb) throws Exception {
        boolean exist = exists(tangle, hash);
        if (hash.equals(Hash.NULL_HASH)) {
            return 0;
        }
        ...
        setMetadata();
        getBytes();
        batch.add(new Pair<>(hash, transaction));
