...
public class Converter {
    ...
    public static byte[] uncompress(byte[] input) throws DataFormatException {
        ...
        try {
            ...
            while (true) {
                decompressor.reset();
                decompressor.setInput(setBytes);
                final byte[] buf = new byte[setBytes.length];
                ...
                if (decompressor.getRemaining() > 0) {
                    int cutOffSet = (int) decompressor.getBytesRead() + 1;
                    setBytes = ArrayUtils.subarray(setBytes, cutOffSet, setBytes.length);
                    bos.write(",".getBytes(), 0, 1);
                } else {
                    break;
                }
            }
